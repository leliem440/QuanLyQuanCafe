...
namespace CafeShopManagementSystem
{
    public partial class RegisterForm : Form
    {
        ...
        private void register_btn_Click(object sender, EventArgs e)
        {
            if(emptyFields())
            {
                MessageBox.Show("ALL fields are required to be filded.","Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                    ...
                    try
                    {
                        ...
                        using (SqlCommand checkUsername = new SqlCommand(selectUsername, connect))
                        {
                            ...
                            if (table.Rows.Count >= 1)
                            {
                                ...
                                MessageBox.Show(usern + "is Alraedy taken", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
